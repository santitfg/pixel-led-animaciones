...
void loop() {
  ...
  if (tiempoActual > marcaTiempoFrec + muestreo) {
    for (int i = 0 ; i < cantPixeles; i++) {
      ...
      float r = ((sin(float(tiempoActual )* frecR + proporcion * PI*2. + desface ) * 0.5 + .5) )* 255.;
      float g = ((sin(float(tiempoActual)* frecG + proporcion * PI*2. ) * 0.5 + .5) )* 255.;
      float b = ((sin(float(tiempoActual)* frecB + proporcion * PI*2.  - desface ) * 0.5 + .5) )* 255.;
